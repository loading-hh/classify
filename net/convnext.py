...
class Net(nn.Module):
    def __init__(self, pretrain, **kwargs):
        ...
        if pretrain == True:
            self.net = torchvision.models.convnext_tiny(weights = torchvision.models.ConvNeXt_Tiny_Weights.DEFAULT)
            self.net.classifier[2] = nn.Linear(self.net.classifier[2].in_features, out_features=3)
        else:
